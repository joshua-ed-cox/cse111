...
def populate_main_window(frm_main):
    ...
    calc_width = Label(frm_main, text="Width:")
    calc_height = Label(frm_main, text="Height:")

    # Create an integer entry box where the user will enter her age.
    ent_width = IntEntry(frm_main, width=4, lower_bound=1)
    ent_height = IntEntry(frm_main, width=4, lower_bound=1)

    # Create a label that displays "Rates:"
    label_area = Label(frm_main, text="Area:")
    ...
    calc_width.grid(row=0, column=0, padx=3, pady=3)
    calc_height.grid(row=0, column=2, padx=3, pady=3)
    ent_width.grid(row=0, column=1, padx=3, pady=3)
    ent_height.grid(row=0, column=3, padx=3, pady=3)

    label_area.grid(row=1, column=0, padx=(30, 3), pady=3)
    ...
    def calculate(event):
        ...
        try:
            # Get the user's age.
            user_width = int(ent_width.get())
            user_height = int(ent_height.get())

            # Compute the user's maximum heart rate.
            area_output = user_width * user_height
            ...
            label_area.config(text=f"{area_output:.0f}")
